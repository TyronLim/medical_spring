...
public class FController {
	...
	public String public_data(@RequestParam(defaultValue = "1") String page) throws Exception {
		// api 호출
		String service_key = "bR8VrEdb3tH42N%2F1DeeaqaY44a39mVTRojVlGP6OOxEvM4CdjD3zBzO%2FGphkmdLD87hDUM%2B0i%2BT722sulnMfJg%3D%3D";
		...
		urlBuilder.append("&" + URLEncoder.encode("MobileOS", "UTF-8") + "=" + URLEncoder.encode("ETC", "UTF-8")); /* 서비스형태 OS구분 (IOS (아이폰), AND (안드로이드), WIN (윈도우폰), ETC(기타)) */
		urlBuilder.append("&" + URLEncoder.encode("MobileApp", "UTF-8") + "=" + URLEncoder.encode("AppTest", "UTF-8")); /* 서비스명(MobileApp) */
		urlBuilder.append("&" + URLEncoder.encode("arrange", "UTF-8") + "=" + URLEncoder.encode("A", "UTF-8")); /*  A=촬영일, B=제목, C=수정일 */
	...
	@ResponseBody
	public String public_data2(@RequestParam(defaultValue = "서울") String keyword) throws Exception {
		...
		urlBuilder.append("&" + URLEncoder.encode("MobileApp", "UTF-8") + "=" + URLEncoder.encode("AppTest", "UTF-8")); /* 서비스명(MobileApp) */
		urlBuilder.append("&" + URLEncoder.encode("arrange", "UTF-8") + "=" + URLEncoder.encode("A", "UTF-8")); /*  A=촬영일, B=제목, C=수정일 */
		urlBuilder.append("&" + URLEncoder.encode("keyword", "UTF-8") + "=" + URLEncoder.encode(keyword, "UTF-8")); /*  키워드 부산  */
		urlBuilder.append("&" + URLEncoder.encode("_type", "UTF-8") + "=" + URLEncoder.encode("json", "UTF-8")); /* 요청자료형식(XML/JSON) */
		URL url = new URL(urlBuilder.toString());
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("GET");
